...
| timeout or connection error | unknown | hold | look up by transaction; settle or release from the answer; never re-pay blind. If the lookup itself fails, or finds the transaction still `pending`, settle at the reserved amount with an `unreconciled` warning; unknown is never free |
